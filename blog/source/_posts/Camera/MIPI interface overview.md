...
![CSI2 Layer](/image/camera/CSI2_Layer.png)
...
![D-PHY Distribution](/image/camera/D-PHY_Distribution.png)
![D-PHY Merging](/image/camera/D-PHY_Merging.png)

C-PHY:

![C-PHY Distribution](/image/camera/C-PHY_Distribution.png)
![C-PHY Merging](/image/camera/C-PHY_Merging.png)
...
![multi-lane n](/image/camera/multi-lane-n.png)

2. M > N, 传输断大于接收端时，就会有性能问题。
![multi-lane m](/image/camera/multi-lane-m.png)
...
![llp overview](/image/camera/llp-overview.png)
...
![D-PHY-LP](/image/camera/D-PHY-LP.png)

#### What does C-PHY Long Packet look like?
![C-PHY-LP](/image/camera/C-PHY-LP.png)
...
![D-PHY-LP](/image/camera/D-PHY-SP.png)

#### What does C-PHY Short Packet look like?
![C-PHY-LP](/image/camera/C-PHY-SP.png)

## Data Identifier (DI)

DI 字节由 VC（Virtual Chanel) 和 DT (Data Type) 两部分组成。如下图：

![DI](/image/camera/DI.png)
...
![VC](/image/camera/VC.png)

交错的数据流格式示例如下：

![Interleaved_Data](/image/camera/Interleaved_Data.png)
> 总共三组
...
> ![Embedded-Data](/image/camera/Embedded-Data.png)
...
![ECC Example](/image/camera/ECC-Example.png)

## Checksum Generation for C-PHY

![Checksum](/image/camera/Checksum.png)
> ECC 在 C-PHY 无效
...
![Packet-Spacing](/image/camera/Packet-Spacing.png)

单包实例如下：
> 图中的 VVALID、HVALID和DVALID 为虚拟的帧同步(VSYNC)、行同步(HSYNC)和有效数据同步（DE，亦可称为数据使能信号）信号。

![Single-Packet](/image/camera/Single-Packet.png)

多包实例如下：  

![Multi-Packet](/image/camera/Multi-Packet.png)
...
![Frame-Line-Blanking](/image/camera/Frame-Line-Blanking.png)

再从行同步和帧同步的角度，来看看帧尾和帧头之间的 LPS （VSYNC 区域），即 Frame Blanking，以及行尾行头之间的 Line Blanking，会更加容易理解。

![HSYNC-VSYNC-Blanking](/image/camera/HSYNC-VSYNC-Blanking.png)
...
![General-Frame-Format](/image/camera/General-Frame-Format.png)

数字隔行视频格式实例：

![Digital-Interlaced-Video](/image/camera/Digital-Interlaced-Video.png)

精确同步的数字隔行视频格式实例如下，未省略行头行尾的短包。

![Digital-Interlaced-Video-with-Accurate-Sync](/image/camera/Digital-Interlaced-Video-with-Accurate-Sync.png)

如下为高速模式下数据和时钟的关系图，可以清楚地了解到高低速序号， LPS 的转变过程。

![Data-Clock](/image/camera/Data-Clock.png)
...
![Interleaved-Data-Transmission](/image/camera/Interleaved-Data-Transmission.png)

按包交错的格式如下：

![Packet-Interleaved-Transmission](/image/camera/Packet-Interleaved-Transmission.png)

按帧交错的格式如下：

![Frame-Interleaved-Transmission](/image/camera/Frame-Interleaved-Transmission.png)

### Virtual Channel Identifier Interleaving

每一个 VC 拥有自己的帧头（FS）和帧尾（FE），而且每一个 VC 还可以继续通过 DT 来扩展数据通路。

![VC-Interleaving](/image/camera/VC-Interleaving.png)
