...
![manual vcs](/image/vcs/media/manual_vcs.png)  
...
![central_vcs](/image/vcs/media/central_vcs.jpeg)
...
![distributed_vcs](/image/vcs/media/distributed_vcs.jpeg)
...
![git_workflow](/image/vcs/media/git_workflow.png)
...
![git_official_doc](/image/vcs/media/git_official_doc.png)
...
![man_git](/image/vcs/media/man_git.png)
...
![git_log](/image/vcs/media/git_log.png)
...
### Fast-forward (--ff)
![git_merge_ff](/image/vcs/media/git_merge_ff.GIF)

### No-fast-foward (--no-ff)
![git_merge_noff_confict](/image/vcs/media/git_merge_noff_confict.GIF)

### Merge Conflicts
![git_merge_noff](/image/vcs/media/git_merge_noff.GIF)
...
![git_reset_hard](/image/vcs/media/git_reset_hard.GIF)

### git reset --soft

![git_reset_soft](/image/vcs/media/git_reset_soft.GIF)
...
![git_rebase_i](/image/vcs/media/git_rebase_i.png)

### rebase branch

![git_rebase_ff](/image/vcs/media/git_rebase_ff.GIF)

### rebase - drop

![git_rebase_drop](/image/vcs/media/git_rebase_drop.GIF)

### rebase - squash

![git_rebase_squash](/image/vcs/media/git_rebase_squash.GIF)
...
![learninggitbranch](/image/vcs/media/learninggitbranch.png)
...
![gitconfig](/image/vcs/media/gitconfig.png)
...
![git_demo_tree](/image/vcs/media/git_demo_tree.png)


## .git/HEAD
`HEAD` 相当于是一个指针，指向当前工作的分支或者 commit。  

![git_head](/image/vcs/media/git_head.png)
...
![git_index](/image/vcs/media/git_index.png)

不过 `Git` 提供了 `ls-files` 命令来查看暂存区中的内容。  

![git_ls_files](/image/vcs/media/git_ls_files.png)

## .git/config

最高优先级的 local configuration, 同一台电脑上的多个项目可以通过此文件进行差异化配置.  

![git_config](/image/vcs/media/git_config.png)


##  .git/refs
这个文件夹里面存储的就是所有的 commit ‘指针’文件。  

![git_refs](/image/vcs/media/git_refs.png)
...
![git_logs](/image/vcs/media/git_logs.png)
...
![git_objects](/image/vcs/media/git_objects.png)

##  commit, tree,blob

`commit, tree, blob` 是 Git 的三个基本单元, 比如我的 Demo 的提交历史如下.  

![git_log_demo](/image/vcs/media/git_log_demo.png)

我们可以通过 `git cat-file -p` 查看 Git 对象的内容，通过 `git cat-file -t` 查看 Git 对象的类型。

### 从第二个 commit (726c6c0) 看进去

![git_commit_content1](/image/vcs/media/git_commit_content1.png)

### 从 HEAD 看进去

![git_commit_content2](/image/vcs/media/git_commit_content2.png)
...
![git_demo_log_graph](/image/vcs/media/git_demo_log_graph.png)

通过 `git checkout 8c19a38` 切到分离头指针。  

![git_detached_head](/image/vcs/media/git_detached_head.png)

`git commit -am` 如下修改。   

![git_detached_head_diff](/image/vcs/media/git_detached_head_diff.png)


`git commit` 后的状态如下图，在切换分支时如果我们不用分支或者 tags 关联此 commit，这部分内容就会被 git 回收掉。 不过很多时候分离头指针在 Git 里还是大有用处的，比如在 rebase 的时候。  

![git_detached_head_push](/image/vcs/media/git_detached_head_push.png)
...
![repo_workflow](/image/vcs/media/repo_workflow.png)
...
![repo_tips](/image/vcs/media/repo_tips.png)

# .repo directory

![repo_dirctory](/image/vcs/media/repo_dirctory.png)
...
![repo_launcher_tool](/image/vcs/media/repo_launcher_tool.png)
...
![repo_manifest](/image/vcs/media/repo_manifest.png)

### manifest format

![repo_manifest_format](/image/vcs/media/repo_manifest_format.png)
...
![repo_project_list](/image/vcs/media/repo_project_list.png)
...
![projects_objects](/image/vcs/media/projects_objects.png)
